...
def predict_on_dataset(model, dataset):
    """Run model on a tf.data.Dataset, return (probabilities, true_labels)."""
    all_probs = []
    all_labels = []
...
def plot_confusion_matrix(y_true, y_pred, model_name="model", normalize=True):
    """Heatmap of artist confusions. Normalised by default (percentages)."""
...
def compute_topk_accuracy(y_true, y_probs, k_values=(1, 3, 5)):
    """Top-k accuracy for k=1,3,5. With 23 artists the 2nd/3rd guess matters."""
    results = {}
    for k in k_values:
        if k >= NUM_CLASSES:
            continue
...
def evaluate_model(model, test_ds, test_paths, test_labels, model_name="model"):
    """Run the full evaluation: predictions, report, confusion matrix, top-k, errors."""
    print(f"\n{'='*60}")
...
def plot_accuracy_vs_class_size(y_true, y_pred, all_labels, model_name="model"):
    """Scatter plot: does having more training images help per-class accuracy?"""
...
def analyse_confusion_clusters(y_true, y_pred, model_name="model", top_n=10):
    """Find the most common artist-pair confusions."""
    cm = confusion_matrix(y_true, y_pred)
...
def plot_model_comparison(results_dict):
    """Grouped bar chart comparing all models across accuracy/top-3/top-5."""
...
# ──────────────────────────────────────────────
# 10. Per-artist F1 comparison
# ──────────────────────────────────────────────
def plot_per_artist_f1(model_reports, filename="per_artist_f1_comparison.png"):
    """Grouped horizontal bar chart comparing F1 per artist across models.
    model_reports: {model_name: (y_true, y_pred)}"""
    ...
    # Sort by last model's F1 for readability
